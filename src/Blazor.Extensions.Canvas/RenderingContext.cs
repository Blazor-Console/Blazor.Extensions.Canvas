...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
    ...
    internal async Task<RenderingContext> InitializeAsync()
    {
        ...
        if (!this._initialized)
        {
            await this._jsRuntime.InvokeAsync<object>(
                identifier: $"{NAMESPACE_PREFIX}.{this._contextName}.{ADD_ACTION}",
                this.Canvas,
                this._parameters);
    ...
    protected async Task BatchCallAsync(string name, bool isMethodCall, params object[] value)
    {
        ...
        Array.Copy(sourceArray: value,
            sourceIndex: 0,
            destinationArray: callObject,
            destinationIndex: 2,
            length: value.Length);
        this._batchedCallObjects.Add(item: callObject);
    ...
    {
        return await this._jsRuntime.InvokeAsync<T>(
            identifier: $"{NAMESPACE_PREFIX}.{this._contextName}.{GET_PROPERTY_ACTION}",
            this.Canvas,
            property);
    }
    ...
    {
        return this.CallMethodAsync<T>(method: method).GetAwaiter().GetResult();
    }

    protected async Task<T> CallMethodAsync<T>(string method)
    {
        return await this._jsRuntime.InvokeAsync<T>(
            identifier: $"{NAMESPACE_PREFIX}.{this._contextName}.{CALL_METHOD_ACTION}",
            this.Canvas,
            method);
    }
    ...
    {
        return this.CallMethodAsync<T>(method: method,
    ...
    private async Task BatchCallInnerAsync()
    {
        ...
        _ = await this._jsRuntime.InvokeAsync<object>(
            identifier: $"{NAMESPACE_PREFIX}.{this._contextName}.{CALL_BATCH_ACTION}",
            this.Canvas,
            currentBatch);
    ...
    public void Dispose()
    {
        Task.Run(function: async () => await this._jsRuntime.InvokeAsync<object>(
            identifier: $"{NAMESPACE_PREFIX}.{this._contextName}.{REMOVE_ACTION}",
            this.Canvas));
    }
